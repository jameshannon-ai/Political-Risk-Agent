...
def _evidence_to_score_bridge(evidence_pack, scores):
    ...
    evidence_by_id = _evidence_rows_by_id(evidence_pack)
    sections = []
    for dimension in ["likelihood", "impact", "immediacy", "confidence"]:
        item = bridge.get(dimension, {})
        trace_item = traceable.get(dimension, {})
        score = trace_item.get("score", item.get("score", scores.get(dimension, {}).get("score", "")))
        source_ids = _score_supporting_source_ids(item, trace_item)
        evidence_rows = [evidence_by_id[source_id] for source_id in source_ids if source_id in evidence_by_id]
        if not evidence_rows:
            evidence_rows = list(evidence_by_id.values())[:3]

        lines = [f"### {dimension.title()}: {score}/5", "", "Evidence used:"]
        for evidence in evidence_rows:
            lines.extend(_format_score_evidence_item(evidence))

        cap_reason = trace_item.get("reason_score_is_capped") or evidence_pack.get("confidence_cap_reason", "")
        if cap_reason:
            lines.extend(["", f"Cap reason: {cap_reason}"])
        sections.append("\n".join(lines))
    return "\n\n".join(sections)


def _evidence_rows_by_id(evidence_pack):
    selected_by_id = {
        source.get("source_id"): source for source in evidence_pack.get("selected_sources", []) if source.get("source_id")
    }
    rows = {}
...
def _score_supporting_source_ids(bridge_item, trace_item):
    source_ids = []
    for evidence in trace_item.get("supporting_evidence", trace_item.get("evidence_supporting_score", [])):
        if isinstance(evidence, str) and evidence:
            source_ids.append(evidence)
        elif isinstance(evidence, dict) and evidence.get("source_id"):
            source_ids.append(evidence["source_id"])
    for source_id in bridge_item.get("supporting_sources", []):
        if source_id not in source_ids:
            source_ids.append(source_id)
    return source_ids


def _format_score_evidence_item(evidence):
    source_id = evidence.get("source_id", "")
    title = _clean_source_title(evidence.get("source_title") or evidence.get("title", "") or "Source")
    publisher = evidence.get("publisher", "")
    mode = evidence.get("evidence_source_mode") or "not_recorded"
    review_note = " (review required)" if mode in {"snippet_only", "metadata_only", "fallback", "manual_input"} else ""
    claim = evidence.get("extracted_claim") or evidence.get("claim_supported") or evidence.get("snippet", "")
    relevance = (
        evidence.get("commercial_relevance")
        or evidence.get("commercial_meaning")
        or evidence.get("business_user_implication")
        or ""
    )
    confidence_effect = evidence.get("confidence_effect") or "supports_score"

    lines = [
        f"- [{source_id}] {publisher or title} — {mode}{review_note}",
        f"  Claim: {_clean_trace_text(claim)}",
    ]
    if relevance:
        lines.append(f"  Commercial relevance: {_clean_trace_text(relevance)}")
    lines.append(f"  Confidence effect: {_clean_trace_text(confidence_effect)}")
    return lines
    ...
    for dimension, item in traceable.items():
        rows.append(
            (
                ...
                _evidence_ids(item.get("weakening_evidence", item.get("contrary_evidence", item.get("evidence_weakening_score", [])))) or "None",
                _evidence_ids(item.get("evidence_quality_limits", [])) or "None",
                ", ".join(item.get("missing_evidence", [])) or "None",
                item.get("confidence_cap_reason", item.get("reason_score_is_capped", "")),
            )
        )
    return _table(["Dimension", "Score", "Label", "Score type", "Supporting evidence", "Weakening evidence", "Evidence quality limits", "Missing evidence", "Cap / review reason"], rows)
